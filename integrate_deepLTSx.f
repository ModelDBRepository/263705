...
! Integration program for superior & deep LTS cells
! From ltsi.f in supergj.f

       SUBROUTINE integrate_deepLTSx (O, time, numcell, V, curr,
     ...
     &  mar)
       ...
       REAL*8 dt, time, Mg, gapcon, cinv(numcomp)
...
7000     CONTINUE
        ...
        CALL DEEPLTSIMAJ (GL,GAM,GKDR,GKA,GKC,GKAHP,GK2,GKM,
...
301    CONTINUE
       ...
       call fnmda (v, open, numcell, numcomp,
     &  Mg, L, A, BB1, BB2)
...
c      do i = 1, ngap_LTS(L)  ! Obsolete gj code from supergj.f
c      L1 = list_gap_LTS(L,i)
c	fv(dendsite) = fv(dendsite) + gapconid_LTS *
c    &   (vdgap_global_LTS(L1) - v(dendsite,L)) * cinv(dendsite)
c      end do                 ! Obsolete gj code from supergj.f
