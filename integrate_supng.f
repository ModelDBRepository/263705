! 16 Sept. 2006, basically the same as integrate_deepng.f
! 31 Aug. 2006, from diego/integrate_deepbask.f, sources also
! bask/lsfs.f and bask/ngint.f, data of Gabor Tamas.
! Neurogialform cell integration subroutine: make basket cell smaller
! and make LS firing properties with increased SD gKA

! Integration program for superior & deep basket & axo-axonic cells
! From baskn.f in supergj.f

       SUBROUTINE integrate_supng   (O, time, numcell, V, curr,
     ...
     &  mar)
       ...
       integer, parameter:: numcomp = 59 
         ! should be compat. with calling prog
...
c as follows:
     ...
     X    alpham_ar , betam_ar , dalpham_ar , dbetam_ar)
        ...
        CALL SUPNGMAJ (GL,GAM,GKDR,GKA,GKC,GKAHP,GK2,GKM,
...
c         gcal_dens(i) = 0.5d0
        ...
        gka_dens(3) =  4.d0
        do i = 4, 9
         gka_dens(i) = 4.0d0 ! in bask.f, these 4's were 1's
        end do
