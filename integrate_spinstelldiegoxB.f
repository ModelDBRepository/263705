c From /isoldeepVFOK/integrate_spinstell_diegox.f
c 14 Aug. 2004: following spinst/bryanpd.f simulations, alter
c spinstell subroutine: equiv. densities of gNaF and gKDR on
c soma vs. axon, AND have gNa rate functions shifted relative
c to soma gNa rate functions (Colbert & Pan, 2002).
! Also make NMDA cond. independent of V and Mg
...
c rectifier (ar).
       ...
       SUBROUTINE integrate_spinstelldiegoxB (O, time, numcell, V, curr,
...
c CINV is 1/C, i.e. inverse capacitance
     ...
     x mkdr(numcomp,numcell), mnap(numcomp,numcell),
...
c this will be gCa conductance corresponding to high-thresh channels
       ...
       real*8 fastna_shift, axon_shift
...
!    X   - (gampa(I,L) + open(i) * gnmda(I,L))*V(I,L)
     X   - (gampa(I,L) +  1.d0   * gnmda(I,L))*V(I,L) ! note
...
!            fastNa_shift = -6.0d0
       ...
       IF (K0.LT.  0) K0 =   0
               ...
               axon_shift = fastNa_shift + 7.d0 
       K2 = IDNINT ( 4.d0 * (V(I,L)+        axon_shift+ 120.d0) )
       IF (K2.GT.640) K2 = 640
       IF (K2.LT.  0) K2 =   0

       IF (i.le.53) then ! SOMA/DENDRITES
        fmnaf(i) = alpham_naf(k0) * (1.d0 - mnaf(i,L)) -
     X              betam_naf(k0) * mnaf(i,L)
        fhnaf(i) = alphah_naf(k0) * (1.d0 - hnaf(i,L)) -
     X              betah_naf(k0) * hnaf(i,L) ! NOTE
       ELSE  ! AXON
        fmnaf(i) = alpham_naf(k2) * (1.d0 - mnaf(i,L)) -
     X              betam_naf(k2) * mnaf(i,L)
        fhnaf(i) = alphah_naf(k2) * (1.d0 - hnaf(i,L)) -
     X              betah_naf(k2) * hnaf(i,L)
       ENDIF
...
c Consistent set of units: nF, mV, ms, nA, microS
        ...
        REAL*8 GNAP(numcomp),GAR(numcomp)
